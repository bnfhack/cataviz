/**
 * © 2009, 2012 frederic.glorieux@fictif.org
 *
...
<input class="suggest" name="search" src="suggest.php?q="/>
*/
if(!Array.isArray) {
...
var Suggest = {
  ...
  create: function (id, src) {
    ...
    if (!src) return alert(id + " : no data source found (provide an url in the @src attribute of the <input>)");
  ...
  show: function(input) {
    input.visible=true;
    if (input.url) {
      // input.url+input.value+'&.js' ? // .js for old flavours of IE
      var url = input.url+input.value;
      var xmlhttp = new XMLHttpRequest();
      xmlhttp.onreadystatechange = function() {
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
          var data = JSON.parse(xmlhttp.responseText);
          Suggest.fill(input, data);
        }
      };
  ...
  fill: function (input, data) {
    ...
    for (var i = 0; i < length; i++) {
      if ( Array.isArray( data[i]) ) {
        html+='<div data-value="'+data[i][0]+'">'+data[i][0]+'</div>';
      } else {
        html+="<div>"+data[i]+"</div>";
      }
    }
